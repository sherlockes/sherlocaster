...
def save_state_bak(state: dict, config=None):
...
def save_state(state: dict, config=None):
    ...
    # --- NUEVO: Gestión de IDs Ignorados (para no repetir comprobaciones) ---
    ignored = state.get("ignored", [])
    # Limitamos la lista a los últimos 1000 descartes para que el JSON no pese megas
    if len(ignored) > 1000:
